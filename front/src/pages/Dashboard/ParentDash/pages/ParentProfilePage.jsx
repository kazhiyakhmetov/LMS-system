...
function ChildLine({ child }) {
  ...
  return (
    <li className={styles.childItem}>
      ...
      <span className={styles.childAvg} title="Средний балл">
        {stats.avgGrade ?? "—"}
      </span>
    </li>
  );
...
export default function ParentProfilePage() {
  ...
  }, [me.fio, profile.firstName, profile.lastName, user?.name]);
        ...
        <div className={styles.profileMain}>
          ...
          <div>
            ...
            <div className={styles.tags}>
              <span className={styles.tag}>Родитель</span>
              {me.childrenCount ? <span className={styles.tag}>{me.childrenCount} детей</span> : null}
            </div>
      ...
      <section className={styles.grid}>
        <article className={styles.panel}>
          ...
          <ul className={styles.rows}>
            ...
            <li className={styles.row}>
              <span className={styles.rowLabel}>Отчество</span>
        ...
        <article className={styles.panel}>
          ...
          <div className={styles.bioActions}>
            ...
            {bioDraft !== null ? (
              <button type="button" className={styles.ghostBtn} onClick={() => setBioDraft(null)}>Отмена</button>
            ) : null}
        ...
        <article className={`${styles.panel} ${styles.widePanel}`}>
          <h3 className={styles.panelTitle}>Мои дети ({children.length})</h3>
          {childrenQuery.loading && !children.length ? (
            <p style={{ color: "var(--muted)", margin: 0 }}>Загрузка…</p>
          ) : children.length ? (
            <ul className={styles.childrenList}>
              {children.map((c) => <ChildLine key={c.id} child={c} />)}
            </ul>
          ) : (
            <p style={{ color: "var(--muted)", margin: 0 }}>
              К вашему аккаунту пока не привязаны дети. Свяжитесь с администратором школы.
            </p>
          )}
        </article>
      </section>
