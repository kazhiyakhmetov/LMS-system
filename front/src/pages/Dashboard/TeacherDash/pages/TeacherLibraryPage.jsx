...
import styles from "./TeacherLibraryPage.module.css";
import { libraryApi } from "../../../../shared/lib/api";
import { useT } from "../../../../shared/lib/i18n";

const SUGGESTIONS = [
  "методика преподавания", "педагогика", "история Казахстана",
  "математика школьная", "физика учебник", "литература хрестоматия",
  "химия", "биология",
];
    ...
    <div className={styles.page}>
      <section className={styles.hero}>
        <div className={styles.heroBody}>
          <span className={styles.heroBadge}>📖 Библиотека для урока</span>
          <h2 className={styles.heroTitle}>Найди книги к уроку</h2>
          <p className={styles.heroSub}>
            Поиск по Open Library — для подбора учебной литературы, цитат и ссылок в задания.
            Можно скопировать ссылку одной кнопкой.
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} className={styles.searchForm}>
          ...
          <input
            ...
            placeholder="Введите название книги, автора или тему урока…"
      ...
      {!loading && hasSearched && results.length === 0 && !error ? (
        <div className={styles.emptyState}>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "var(--muted)" }}>
            Попробуйте переформулировать или поискать по имени автора
          </p>
        </div>
      ) : null}
        ...
        <section className={styles.results}>
          ...
          <ul className={styles.resultList}>
            {results.map((item, idx) => {
              const key = item.url || item.title;
              return (
                <li key={`${key}-${idx}`} className={styles.resultItem}>
                  ...
                  <div className={styles.resultActions}>
                    ...
                    >
                      {copied === key ? "✓ Скопировано" : "📋 Скопировать ссылку"}
                    </button>
                    {item.url ? (
                      <a
                        href={item.url}
